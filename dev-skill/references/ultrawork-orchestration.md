...
- Gate authorization
- Failure recovery
- Workflow-engine repair instructions
...
Apply these instructions to every UltraWork stage and coordinator-level agent call. They are operating rules, not a report of a prior run.

1. **Budget before dispatch.** Design each stage to finish within approximately eight model iterations. If the work may exceed that budget, split it into smaller dependency-linked stages before dispatch. Do not re-spawn an oversized stage unchanged.
2. **Verify artifact-producing stages.** A stage that writes a file or other artifact is complete only after a read-back or digest check proves that the artifact exists and matches the expected output shape. A text-only completion message is never sufficient. Include the verification step in the stage contract.
3. **Resume failures instead of recreating runs.** For a runtime stage failure, resubmit the same script with `resumeFromRunId` so completed results are replayed and only failed or changed work runs. If the same stage fails twice, split it into smaller stages and revise the plan; never run the unchanged stage a third time.
4. **Repair script-load defects in place.** A syntax error, interpolation error, or `ReferenceError` before stage execution is a generated-script defect. Fix the smallest defect, verify that the corrected script parses, and resume the same run. In generated JavaScript, do not place raw `${NAME}` values inside backtick prompts; escape them or substitute values through a function or `.replace()` call.
5. **Require structured terminal output.** A stage that promises structured data is complete only when its final turn emits a valid `StructuredOutput` call or parseable JSON final text. Treat fallback-parse errors and text-only returns as failed stages; fix the prompt or script, then resume or split.
6. **Protect orchestrator context.** Keep the coordinator below its context guard (target approximately 50K tokens of history) by checkpointing per roadmap item and delegating bounded file reads. Repeated pruning, unfinished streams, or a missing finish marker is evidence that the plan or session is too large: checkpoint, hand off to a fresh session, split the work, and resume rather than repeatedly retrying the same session.

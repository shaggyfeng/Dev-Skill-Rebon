...
| `family_id` | `plan` |
| `stage` | `2` |
| `input` | approved Design packet, repository snapshot, constraints, current environment |
...
| architecture evidence insufficient | `improve-codebase-architecture` | one bounded survey and reclassification |
...
| held Work-start packet requires fresh environment or profile validation | `plan.held.revalidate` | revalidated handoff, hold, or invalidation |
...
3. Emit the initial Plan graph, classify architecture, and run one bounded survey when selected.
4. Route architecture surveys to `plan.architecture.classify`; when a proposal is required, route it through `codebase-design` before human resolution.
5. Resolve architecture inside accepted Design meaning.
6. Compile the specification and bind every requirement to observable acceptance and a consumer.
7. Atomize the ticket DAG.
8. Resolve specification and ticket set through their distinct tagged resolver rows; compile the Work-start candidate from the approved ticket set, then resolve it through the Work-start resolver; never combine their inputs or approvals.
9. Emit a Work handoff, hold, revision, rejection, or Design reopen.
10. Re-enter Stage 0 and Plan before each consumer transition.
...
Every ticket binds exact scope, transformation, dependencies, blockers, write ownership, review surface, verification, recovery, resume pointer, and next consumer. The graph is acyclic. A Work handoff contains only approved scope and derived fields.

## State machine

```text
design_handoff -> architecture_classified | clarification_required | design_reopened
architecture_classified -> architecture_resolution | specification_candidate
architecture_resolution -> architecture_resolved | plan_revision | design_reopened
architecture_resolved -> specification_candidate
specification_candidate -> specification_resolution | plan_revision | plan_rejected
...
| `to-tickets` | `plan.ticket-generation-input.v0.1` containing `plan.specification.v0.1` with `status=specification_approved` plus current `plan.graph.v0.1` | `plan.ticket-set.v0.1` with `status=ticket_set_candidate` plus successor `plan.graph.v0.1` | Candidate Work decomposition only | `successor-v0/modules/to-tickets.md` | `ticket_repartition` or `plan_revision` | `plan.ticket-set.resolve` |
...
| `plan.ticket-set.resolve` | `plan.ticket-set.v0.1` with `status=ticket_set_candidate` plus `plan.ticket-set-answer.v0.1` plus current `plan.graph.v0.1` | successor `plan.ticket-set.v0.1` with `status=ticket_set_approved` and `plan.ticket-set-resolution.v0.1` plus successor `plan.graph.v0.1` | Exact human ticket-set decision only | `family-local` | Repartition, Plan revision, or clarification | `plan.work-start.compile` for the approved branch or Plan terminal compiler for non-approved terminals |
| `plan.work-start.compile` | `plan.ticket-set.v0.1` with `status=ticket_set_approved` plus current `plan.graph.v0.1`, `plan.environment-snapshot.v0.1`, and `development-execution-profile.v0.1` | `plan.work-start-candidate.v0.1` with approved ticket-set digest, environment/profile bindings, exclusions, authority, resume pointer, and consumer | Candidate compilation only | `family-local` | Hold, Plan revision, or clarification | `plan.work-start.resolve` |
| `plan.work-start.resolve` | `plan.ticket-set.v0.1` with `status=ticket_set_approved` plus `plan.work-start-candidate.v0.1` plus `plan.work-start-answer.v0.1` plus current `plan.graph.v0.1` | `plan.work-start-resolution.v0.1` with `status=work_start_authorized` or `work_start_held` plus successor `plan.graph.v0.1`; the held branch also emits `plan.held-packet.v0.1` preserving environment/profile bindings and `plan.resume-pointer.v0.1` | Exact human Work-start authorization only | `family-local` | Hold, Plan revision, or clarification | Handoff, held revalidation, or Plan terminal compiler |
| `handoff` | `runtime.boundary-input.v0.1` containing `plan.specification.v0.1` with `status=specification_approved`, `plan.ticket-set.v0.1` with `status=ticket_set_approved`, and `plan.work-start-resolution.v0.1` with `status=work_start_authorized` | `runtime.handoff.v0.1` binding the immutable `plan.work-handoff.v0.1` | Context transfer only; no approval or Work authority | `successor-v0/modules/handoff-and-boundaries.md` | `handoff_blocked` or `boundary_revalidation_required` | Work entry |
| `plan.held.revalidate` | `plan.held-packet.v0.1` plus `plan.environment-snapshot.v0.1` plus `development-execution-profile.v0.1` plus `plan.work-start-request.v0.1` plus current `plan.graph.v0.1` | `plan.held-revalidation.v0.1` plus optional `plan.work-handoff.v0.1` plus successor `plan.graph.v0.1` | Revalidation and exact newly resolved Work scope only | `family-local` | Invalidation, Plan revision, or Design reopen | Handoff or terminal compiler |
...
- Plan cannot change accepted Design meaning or authority.

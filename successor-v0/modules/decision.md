...
|---|---|---|
| Indirect intent or a frozen semantic candidate is available | `decision_started` | Human Intent framing or semantic evaluation within Decision's authority |
| Decision has a stated remaining gap or disposition | `decision_returned` | Declared caller or Route consumer; Decision does not advance it |
...
1. At `decision_started`, for indirect intent with no directly determined operation, state only the current observation, desired outcome, protected meaning, and unresolved ambiguity. Return Human Intent State, a factual gap, or a human-owned question. Do not invent facts, candidate options, a stage, a module, or authority.
...
The caller invokes Decision only when its current goal, accepted constraints, and declared consumer do not uniquely determine the next operation. Decision returns one of the following and never advances a host checkpoint itself:

| Remaining result | Consumer |
|---|---|
| Determined disposition | Calling operation |
| Factual gap | Research through the caller or Route |
| Experience gap | Prototype or human through the caller |
| Human-owned meaning | Grilling |
| Semantic disposition that changes stage or module | Route |

When no semantic fork remains, the caller follows its declared consumer mechanically. Do not create a new candidate, question, or Decision call merely because an operation has more steps.
...
| Actionable Review finding or correction | Calling review or closure owner |
| Invalidated Review finding or correction | Calling review or closure owner; no action |
...
- Decision may invalidate a Review finding or correction as an action request; it does not alter the finding's evidence.
